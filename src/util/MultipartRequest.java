...
public class MultipartRequest implements HttpServletRequest {
	...
	public MultipartRequest(HttpServletRequest req, long userId)
			throws IOException {
		if (req == null)
	...
	private boolean readNextPart(MultipartInputStreamHandler in,
		...
		if (strExactFileName == null) {
			...
			ArrayList<String> existingValues = (ArrayList<String>) parameters_
			...
			if (!strExactFileName.equals("unknown")) {
				String ext = "";
	            int index = strExactFileName.lastIndexOf(".");
	            if(index > 0){
	                ext = strExactFileName.substring(index).toLowerCase();
	            }
	...
	private void readAndSaveFile(MultipartInputStreamHandler in,
	...
	private String readParameter(MultipartInputStreamHandler in,
			String boundary) throws IOException {
